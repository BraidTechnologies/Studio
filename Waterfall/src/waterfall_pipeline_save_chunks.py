...
from CommonPy.src.chunk_repository_api_types import (
    IStoredChunk, create_text_rendering, create_embedding)
...
def save_chunk_tree(output_location: str,
    ...
    # Save the Theme
    master_theme.url = "https://braid-api.azurewebsites.net/api/GetPage?id=" + \
        str(master_theme.id)

    master_theme_embedding = embedder.embed_text(master_theme.originalText)
    master_theme.storedEmbedding = create_embedding(
        master_theme_embedding, chunk_repository.default_embedding_model)
    master_theme.storedSummary = create_text_rendering(
        master_theme.originalText, chunk_repository.default_model)
    master_theme.storedTitle = create_text_rendering(
        spec.description, chunk_repository.default_model)
