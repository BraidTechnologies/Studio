# Generated by ts2python version 0.7.5 on 2025-01-17 18:21:01.221360
...
source_hash__ = "94a48e86d420b5958c784a067a21e454"


##### BEGIN OF ts2python generated code


class IEnrichedChunkSummary(TypedDict, total=True):
    url: str
    text: str
    summary: str


class IEnrichedChunk(IEnrichedChunkSummary, TypedDict, total=True):
    id: str
    embedding: List[float]


class IRelevantEnrichedChunk(TypedDict, total=True):
    chunk: IEnrichedChunkSummary
    relevance: float


class IChunkQuerySpec(TypedDict, total=True):
    repositoryId: 'EChunkRepository'
    maxCount: float
    similarityThreshold: float


class IChunkQueryRelevantToUrlSpec(IChunkQuerySpec, TypedDict, total=True):
    url: str


class IChunkQueryRelevantToSummarySpec(IChunkQuerySpec, TypedDict, total=True):
    summary: str


class IEnrichedQueryRequest(TypedDict, total=True):
    repositoryId: 'EChunkRepository'
    similarityThreshold: float
    maxCount: float
...
class IEnrichedResponse(TypedDict, total=True):
    answer: str
    chunks: List[IRelevantEnrichedChunk]


class IGenerateQuestionRequest(TypedDict, total=True):
    summary: str
    wordTarget: float


class IQuestionGenerationResponse(TypedDict, total=True):
    question: str
