...
def run_evaluations(eval_dirs=None):
    ...
    Args:
        eval_dirs: Optional list of directories containing eval files.
                  If None, defaults to the evals directory.
    """
    if eval_dirs is None:
        eval_dirs = [project_root / "evals"]
    ...
    results_dir = project_root / "prompt_coverage_results" / datetime.now().strftime("%Y%m%d_%H%M%S")
