...
def test_studio_boxer_response_enrichment_structure(enrichment_data):
...
@pytest.mark.skip(reason='Helper function, not a test')
def test_i_studio_boxer_response_structure(response_data):
    ...
    for enrichment in response_data:
        test_studio_boxer_response_enrichment_structure(enrichment)
...
def test_studio_boxer():
    ...
    response = requests.post(
        studio_boxer_url, params=params, timeout=request_timeout)
    assert response.status_code == 200
    response_json = response.json()

    # Validate the response against the schema
    validate_response_vs_schema(response_schema, response_json)
    test_i_studio_boxer_response_structure(response_json)
...
def test_invalid_studio_request():
    ...
    response = requests.post(
        studio_boxer_url, params=invalid_data, timeout=request_timeout)
